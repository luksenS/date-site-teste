...
export default function EnchantingDateProposalApp() {
  ...
  const steps = [
    ...
    <motion.div key="step0" className="text-center" {...fadeInUp}>
      ...
      <motion.img
        ...
        transition={{ duration: 0.5 }}
        src="/doguinhos auau.jpeg"
        alt="cachorrinhos fofinhos"
          ...
          <DialogContent className="bg-pink-50 border-2 border-pink-300">
            <DialogHeader>
              <DialogTitle className="text-2xl font-bold text-pink-600">
                nao nao nao nao, nao existe nao!
              </DialogTitle>
              <DialogDescription className="text-lg text-pink-500">
                VOCE TEM Q VIR COMIGO!
              </DialogDescription>
            </DialogHeader>
            <motion.video
              initial={{ opacity: 0, scale: 0.8 }}
    ...
    <motion.div key="step1" className="text-center" {...fadeInUp}>
      ...
      >
        <Clock className="mr-2 h-5 w-5" /> escolher a data!{" "}
    ...
    <motion.div key="step2" className="text-center" {...fadeInUp}>
      ...
      <h2 className="text-4xl sm:text-5xl font-playfair font-bold mb-8 bg-clip-text text-transparent bg-gradient-to-r from-rose-500 to-pink-600">
        o que voce quer comer, meu amo?
      </h2>
      <div className="grid grid-cols-2 gap-4 md:gap-6 mb-8">
        {[
          { name: "sorvetinho", icon: <IceCream className="w-6 h-6" /> },
          { name: "lanchinho", icon: <Sandwich className="w-6 h-6" /> },
          { name: "almocinho", icon: <UtensilsCrossed className="w-6 h-6" /> },
          { name: "jantarzinho", icon: <UtensilsCrossed className="w-6 h-6" /> },
          { name: "cervejinha", icon: <Wine className="w-6 h-6" /> },
        ].map(({ name, icon }) => (
          <SelectButton
            key={name}
            icon={icon}
    ...
    <motion.div key="step3" className="text-center" {...fadeInUp}>
      <h2 className="text-3xl font-semibold mb-6 bg-clip-text text-transparent bg-gradient-to-r from-rose-500 to-pink-600">
        O que vamos fazer depois de cume?
      </h2>
      <div className="grid grid-cols-2 gap-6 mb-6">
        {[
          { name: "ver filminho", icon: <Film className="mx-auto mb-2" /> },
          { name: "fazer caminhada", icon: <Footprints className="mx-auto mb-2" /> },
          { name: "ir no parque", icon: <Trees className="mx-auto mb-2" /> },
          { name: "karaoke cantar bts e calcinha preta", icon: <Mic2 className="mx-auto mb-2" /> },
          { name: "jogar boliche", icon: <Target className="mx-auto mb-2" /> },
          { name: "fazer um cpf novo rs", icon: <Baby className="mx-auto mb-2" /> },
          { name: "deixar a vida levar!", icon: <Smile className="mx-auto mb-2" />},
      ].map(({ name, icon }) => (
        <motion.button
          key={name}
  ...
  >
    ...
    <motion.div key="step4" className="text-center" {...fadeInUp}>
      ...
      <h2 className="text-4xl sm:text-5xl font-playfair font-bold mb-8 bg-clip-text text-transparent bg-gradient-to-r from-rose-500 to-pink-600">
        o quao feliz voce esta para o nosso proximo date?
      </h2>
      ...
      >
        Nivel de felicidade: <span className="text-rose-500">{answers.excitement}%</span>
      </motion.div>
      <Button
        onClick={() => {
          setStep(step + 1);
      ...
      >
        seu viadinho vamo oficializar entao! 💕
      </Button>
      </StepCard>
    </motion.div>,
    ...
    <motion.div key="step5" className="text-center" {...fadeInUp}>
      ...
      >
        <p className="text-base">vamos comer/tomar muito(a) <span className="font-semibold">{answers.food.join(", ")}</span>.</p>
        <p className="text-base">Depois a gente vai <span className="font-semibold italic">&quot;{answers.movie}&quot;</span> juntos.</p>
        <p className="text-xl font-playfair font-bold mt-6">
          Nivel de felicidade: <span className="text-rose-600">{answers.excitement}/100</span>
        </p>
      </motion.div>
      </StepCard>
    </motion.div>,
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-6" style={{background: "#eaddfd"}}>
